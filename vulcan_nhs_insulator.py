...
def execute_vulcan_allocation_matrix():
    ...
    Maps real-time patient load bottlenecks, evaluates incident priority scores, and exports telemetry.
    """
    ...
    print("==========================================================================")
    print("Clinic Node         | Active Load | Bottleneck | Priority Index | Direct Routing")
    print("--------------------------------------------------------------------------")
    
    # 4-Node footprint tracking array expanded with real-time surge variance indices
    regional_clinics = {
        "Warrington Central": {"load": 142.5, "capacity": 150.0, "surge_variance": 1.05},
        "Salford Frontier   ": {"load": 118.9, "capacity": 160.0, "surge_variance": 0.95},
        "Halton Urgent Care ": {"load": 165.2, "capacity": 140.0, "surge_variance": 1.35}, # High Priority
        "Cheshire East Hub  ": {"load": 134.0, "capacity": 150.0, "surge_variance": 1.10}
    }
    ...
    for node, metrics in regional_clinics.items():
        ...
        surge = metrics["surge_variance"]
        
        # Calculate localized structural bottleneck and real-time priority scores
        bottleneck_index = active_load / capacity
        priority_index = bottleneck_index * surge
        total_load += active_load
        
        # Direct allocation multi-physics distribution calculation
        direct_allocation = (total_funding_pool_mw_equiv / len(regional_clinics)) * (priority_index ** 1.5)
        ...
        print(f" {node} | {active_load:.1f} Pnts   | {bottleneck_index:.4f}     | {priority_index:.4f}         | {direct_allocation:.2f} MW")
